...
const getDiffAssetList = async (options: RequestInit, appVersion: string) => {
  ...
  const listAssets = (raw: string) => raw.split('\n').filter(Boolean).map((line) => {
    ...
    const fileName = filePath.match(/(.+)\/(?<fileName>\w+)_(\w+)\.bundle/)?.groups?.fileName;
    return { fileName, filePath, hashCode, fileSize: Number(fileSize) };
  });
  ...
  if (res.ok) {
    ...
    const differentAssetList = _.differenceWith(currentAssetList, previousAssetList, (a, b) => {
...
const main = async () => {
  ...
  for await (const asset of assetList) {
    ...
    console.log(`[${index} / ${assetList.length}] ${asset.filePath} is downloading`);
